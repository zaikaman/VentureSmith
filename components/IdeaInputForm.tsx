            ...
            <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
                <div className={`relative transition-all duration-300 ${isFocused ? 'scale-105' : 'scale-100'}`}>
                    <textarea
                        ...
                        className="w-full h-32 p-4 text-lg bg-slate-800 border-2 border-slate-700 rounded-xl focus:ring-4 focus:ring-indigo-500/50 focus:border-indigo-500 focus:outline-none transition-all duration-300 resize-none text-slate-100"
                    />
                </div>
                <button
                    type="submit"
                    disabled={!idea.trim()}
                    className="mt-8 inline-flex items-center gap-3 text-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 py-4 px-10 rounded-full shadow-lg hover:scale-105 transform transition-transform duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
                >
