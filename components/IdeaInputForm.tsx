...
export const IdeaInputForm: React.FC<IdeaInputFormProps> = ({ onGenerate }) => {
    ...
    return (
        <div className="text-center p-8 transition-all duration-500 ease-in-out">
            <h1 className="text-5xl md:text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-sky-400 mb-4">
                AI Startup Builder
            </h1>
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
                <div className="relative">
                    <textarea
                        ...
                        className="w-full h-36 p-5 text-lg bg-gray-900/50 border border-gray-700/50 rounded-2xl focus:ring-2 focus:ring-purple-500/60 focus:border-purple-500 focus:outline-none transition-all duration-300 resize-none text-slate-100 placeholder-gray-500"
                    />
                </div>
                <button
                    type="submit"
                    disabled={!idea.trim()}
                    className="mt-8 inline-flex items-center gap-3 text-xl font-bold text-white bg-indigo-600 py-4 px-10 rounded-full shadow-lg hover:bg-indigo-500 hover:scale-105 transform transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed disabled:scale-100"
                >
