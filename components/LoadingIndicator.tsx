    ...
    return (
        <div className="flex flex-col items-center justify-center text-center p-8 animate-fade-in">
            <div className="relative w-24 h-24 mb-8">
                <div className="absolute inset-0 bg-gradient-to-r from-purple-600 to-sky-500 rounded-full animate-spin opacity-80"></div>
            ...
            <h2 className="text-3xl font-bold text-white mb-2">Building Your Startup</h2>
            <p className="text-gray-400 max-w-xl mb-8">"{idea}"</p>
            <div className="w-full max-w-md">
                <p className="text-lg text-sky-300 font-medium h-8 transition-opacity duration-500">{loadingSteps[currentStepIndex]}</p>
                 <div className="h-2 bg-gray-800 rounded-full overflow-hidden mt-4">
                    <div className="h-full bg-gradient-to-r from-purple-500 to-sky-500" style={{ width: `${((currentStepIndex + 1) / loadingSteps.length) * 100}%`, transition: 'width 1.8s cubic-bezier(0.4, 0, 0.2, 1)' }}></div>
                </div>
            </div>
        </div>
    );
