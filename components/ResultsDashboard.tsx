    ...
    return (
        <div className="w-full animate-fade-in-up">
            <header className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                <h1 className="text-4xl font-bold text-white text-center sm:text-left">Your Startup Blueprint</h1>
                <button
                    onClick={onReset}
                    className="bg-gray-800/80 text-gray-300 font-bold py-2 px-4 rounded-lg hover:bg-gray-700 transition-colors duration-300 flex items-center gap-2 border border-gray-700"
                >
             ...
             <div className="mb-6">
                <div className="bg-gray-900/60 p-2 rounded-xl border border-gray-800/50 backdrop-blur-lg">
                    <nav className="flex items-center justify-start sm:justify-center space-x-2 overflow-x-auto" aria-label="Tabs">
                         {tabs.map((tab) => (
                            <button
                                key={tab}
                                ...
                                } whitespace-nowrap py-3 px-5 rounded-lg font-medium text-sm flex items-center gap-2 transition-all duration-300`}
                            >
                                {icons[tab]} {tab}
                            </button>
                        ))}
            ...
            <div className="p-1 rounded-2xl bg-gradient-to-b from-gray-800/50 to-transparent">
                 <div className="bg-gray-900/80 p-4 sm:p-6 md:p-8 rounded-[14px] backdrop-blur-xl border border-gray-800/50">
                    {renderTabContent()}
                </div>
            </div>
        </div>
