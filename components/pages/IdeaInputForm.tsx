...
export const IdeaInputForm: React.FC<IdeaInputFormProps> = ({ onGenerate }) => {
    ...
    return (
        <div className="text-center p-8 transition-all duration-500 ease-in-out">
            <p className="text-lg md:text-xl text-[var(--text-slate-400)] mb-10 max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
                <div className={`relative transition-all duration-300 ${isFocused ? 'scale-105' : 'scale-100'}`}>
                    <textarea
                        ...
                        className="w-full h-32 p-4 text-lg bg-[var(--bg-slate-800)] border-2 border-[var(--border-slate-700)] rounded-xl focus:ring-4 focus:ring-[var(--primary-color)]/50 focus:border-[var(--primary-color)] focus:outline-none transition-all duration-300 resize-none text-[var(--text-slate-100)]"
                    />
                </div>
                <button
                    type="submit"
                    disabled={!idea.trim()}
                    className="mt-8 inline-flex items-center gap-3 text-xl font-bold text-white bg-gradient-to-r from-[var(--gradient-from)] to-[var(--gradient-to)] py-4 px-10 rounded-full shadow-lg hover:scale-105 transform transition-transform duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
                >
