...
const GenerateMarketingCopy: React.FC<GenerateMarketingCopyProps> = ({ startup }) => {
  ...
  const renderInitial = () => (
    <div className="initial-view">
        <h3 className="text-3xl font-bold mb-4 text-white">Generate AI-Powered Marketing Copy</h3>
        <p className="text-slate-300 mb-8 max-w-3xl mx-auto">
            Create a complete set of marketing materials, from taglines to social media posts and ad copy, all tailored to your brand and target audience.
        </p>
        <button
        ...
        {!canGenerate && <p className="text-sm text-slate-500 mt-4">Please complete Brand Identity, Customer Personas, and Pricing Strategy first.</p>}
    </div>
  );
    ...
    <div className="marketing-copy-container">
      {hasContent && (
        <div className="header-section">
            <h2 className="text-3xl font-bold">Marketing Copy</h2>
            {copyData && !isGenerating && (
                <div className="header-actions">
                    <button onClick={handleGenerate} className="regenerate-button" title="Regenerate Marketing Copy">
      ...
      {isGenerating ? renderLoading() : copyData ? renderResults() : renderInitial()}
    </div>
  );
};
