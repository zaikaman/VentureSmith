...
export const setupIdentifyGrowthMetrics = action({
  args: {},
  handler: async () => {
    console.log("--- Starting Scorecard.ai Identify Growth Metrics Evaluation Setup ---");
    ...
    try {
      ...
      console.log("Creating new project for Identify Growth Metrics...");
      const project = await client.projects.create({
        name: "VentureSmith Identify Growth Metrics Evaluation",
        description: "Automated evaluation for the Identify Growth Metrics feature."
      });
      ...
      const metricsToCreate = [
        { name: "Relevance", description: "Are the suggested metrics relevant to the startup's business model and goals?" },
        { name: "Actionability", description: "Are the metrics actionable and can they be realistically tracked?" },
        { name: "Clarity", description: "Are the metrics clearly defined and easy to understand?" },
      ];
      ...
      const createdMetrics = await Promise.all(
        metricsToCreate.map(metric => 
          client.metrics.create(project.id, {
            ...
            evalType: 'ai',
            outputType: 'boolean',
            promptTemplate: `Is the following set of growth metrics good in terms of ${metric.name}? Answer with only 'true' or 'false'. Content: {{outputs.output}}`,
          })
        )
      );
      ...
      const testset = await client.testsets.create(project.id, {
        name: "Identify Growth Metrics",
        description: "Test cases for the growth metrics generation feature.",
      ...
      await client.testcases.create(testset.id, {
        items: [
            {
                jsonData: {
                    fullContext: "{\"name\":\"Test SaaS Startup\",\"refinedIdea\":\"A platform for managing online communities.\"}",
                }
            }
        ]
      });
...
`;
      ...
      console.log("\n--- ✅ Identify Growth Metrics Setup Complete! ---");
