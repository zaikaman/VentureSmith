...
export const setupMarketPulseCheck = action({
  args: {},
  handler: async () => {
    console.log("--- Starting Scorecard.ai Market Pulse Check Evaluation Setup ---");
    ...
    try {
      ...
      console.log("Creating new project for Market Pulse Check...");
      const project = await client.projects.create({
        name: "VentureSmith Market Pulse Evaluation",
        description: "Automated evaluation for the Initial Market Pulse Check feature."
      });
      ...
      const metricsToCreate = [
        { name: "Data Richness", description: "Does the summary contain concrete data points (scores, keywords)?" },
        { name: "Insightfulness", description: "Is the summary insightful and not just generic statements?" },
        { name: "Conciseness", description: "Is the summary brief and to the point?" },
      ];
      ...
      const createdMetrics = await Promise.all(
        metricsToCreate.map(metric => 
          client.metrics.create(project.id, {
            ...
            evalType: 'ai',
            outputType: 'boolean',
            promptTemplate: `Is the following market pulse check output good in terms of ${metric.name}? Answer with only 'true' or 'false'. Content: {{outputs.output}}`,
          })
        )
      );
      ...
      const testset = await client.testsets.create(project.id, {
        name: "Initial Market Pulse Check",
        description: "Test cases for the market pulse check feature.",
      ...
      await client.testcases.create(testset.id, {
        items: [
            {
                jsonData: {
                    idea: "A subscription box for rare indoor plants.",
                }
            }
        ]
      });
...
`;
      ...
      console.log("\n--- ✅ Market Pulse Check Setup Complete! ---");
