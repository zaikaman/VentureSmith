...
export const setupSeoStrategy = action({
  args: {},
  handler: async () => {
    console.log("--- Starting Scorecard.ai SEO Strategy Evaluation Setup ---");
    ...
    try {
      ...
      console.log("Creating new project for SEO Strategy...");
      const project = await client.projects.create({
        name: "VentureSmith SEO Strategy Evaluation",
        description: "Automated evaluation for the Generate SEO Strategy feature."
      });
      ...
      const metricsToCreate = [
        { name: "Keyword Relevance", description: "Are the suggested keywords highly relevant to the startup's product and target audience?" },
        { name: "Search Intent Alignment", description: "Does the strategy correctly identify and target different search intents (informational, transactional, etc.)?" },
        { name: "Content Strategy Cohesion", description: "Do the suggested content topics logically flow from the keyword clusters?" },
      ];
      ...
      const createdMetrics = await Promise.all(
        metricsToCreate.map(metric => 
          client.metrics.create(project.id, {
            ...
            evalType: 'ai',
            outputType: 'boolean',
            promptTemplate: `Is the following SEO strategy good in terms of ${metric.name}? Answer with only 'true' or 'false'. Content: {{outputs.output}}`,
          })
        )
      );
      ...
      const testset = await client.testsets.create(project.id, {
        ...
        description: "Test cases for the SEO strategy generation feature.",
      ...
      await client.testcases.create(testset.id, {
        items: [
            {
                jsonData: {
                    fullContext: "{\"name\":\"Test Local Bakery\",\"refinedIdea\":\"A bakery specializing in gluten-free cakes.\"}",
                }
            }
        ]
      });
